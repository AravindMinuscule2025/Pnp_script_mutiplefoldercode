...
$listId = "e382e551-213d-46ab-87cd-b292045a5c26"
$listItemsApi = "https://graph.microsoft.com/v1.0/sites/$siteId/lists/$listId/items?expand=fields&top=5000"
...
$libraryDriveId = "b!BzAxdNAlSUOpTE-0cKY5cx_xrr1o_f1FhUdZHct2g2vuhaqgu_zcRY_kIljueCMb"
...
foreach ($siteGroup in $listItemsnew) {
    ...
    #   Get site-specific drive ID
    $matchSourceDrive = $allSiteItems | Where-Object { $_.Title -eq "Europe_source_files_New" }
    if ($null -eq $matchSourceDrive) {
        Write-Host "Source drive 'Europe_source_files_New' not found. Skipping site $siteUrl" -ForegroundColor Red
        continue
    }
    ...
    foreach ($item in $groupItems) {
        ...
        #Access token generation if expired 
        $now = Get-Date
        ...
        # Process each item file details
        try {
            $Title = $item.Title
            ...
            $Status = $item.Status
            Write-Host "Now processing $Title ->$Foldercode -> $siteUrl" -ForegroundColor Cyan
            #  target folder name
            $foldername = "property-$Foldercode"

            # get file metadata id and size from targetFileNames hashtable
            ...
            #less then 50 mb put method or else copy api
            if ($fileSize -le 50MB) {
        ...
        if ($allSucceeded) {
            ...
            # Example log update
            $logApi = "https://graph.microsoft.com/v1.0/sites/root:/sites/Europe:/lists/Processedfileslog_New/items"
